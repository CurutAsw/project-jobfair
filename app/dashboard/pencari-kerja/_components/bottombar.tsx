...
const navItems = [
  { id: 'home', label: 'Home', image: '/dashboard-images/nav-home.svg' },
  ...
  return (
    <nav className={`fixed bottom-0 left-0 right-0 h-16 border-t flex items-center justify-around px-2 z-50 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-gray-200'}`}>
      {navItems.map((item) => {
        ...
        return (
          <button key={item.id} type="button" onClick={() => setActiveTab(item.id)} className="flex flex-col items-center justify-center w-20 h-full relative gap-1">
