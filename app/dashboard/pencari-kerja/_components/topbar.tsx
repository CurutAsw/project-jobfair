...
export default function TopBar({ activeTab, onProfileClick, onMessengerClick}: TopBarProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-16 border-b flex items-center justify-between px-4 z-50 transition-colors duration-300 bg-white border-gray-200 text-gray-900">
      
      {/* --- TOMBOL SIDEBAR (GARIS 3) --- */}
      <button 
        type="button"
        onClick={onProfileClick}
        className="flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center transition-colors hover:bg-gray-100 text-gray-800"
      >
      ...
      <div ref={dropdownRef} className="relative w-full max-w-xl mx-4 flex items-center">
        ...
        <input 
          ...
          className="w-full pl-9 pr-4 py-2 rounded-full text-xs border focus:outline-none transition-colors bg-gray-100 border-gray-200 text-gray-800 placeholder-gray-400 focus:border-gray-300"
        />

        {/* --- DROPDOWN REKOMENDASI PENCARIAN --- */}
        {isFocused && (
          <div className="absolute top-full left-0 right-0 mt-2 p-4 rounded-2xl shadow-xl border text-xs z-50 max-h-80 overflow-y-auto bg-white border-gray-100 text-gray-800">
            {searchQuery.trim() === '' ? (
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {rekomendasiKategori.map((kat, index) => (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-full font-medium transition-colors bg-gray-100 hover:bg-gray-200 text-gray-600"
                      >
                        {kat}
                      </button>
                    ))}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
                    {rekomendasiPerusahaan.map((pt, index) => (
                      <button
                        ...
                        className="text-left px-3 py-2 rounded-xl font-medium transition-colors hover:bg-gray-50 text-gray-700"
                      >
                        {pt}
                      </button>
                    ))}
              ...
              <div>
                ...
                <div className="space-y-0.5">
                  <button 
                    type="button"
                    className="w-full text-left px-3 py-2 rounded-xl font-medium transition-colors hover:bg-gray-50"
                  >
      ...
      <button 
        ...
        className="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center text-lg relative transition-colors hover:bg-gray-100"
      >
        💬
