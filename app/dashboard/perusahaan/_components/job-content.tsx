...
const companyJobs: CompanyJob[] = [
  { id: 1, title: 'Software Engineer 1', status: 'Aktif' },
  { id: 2, title: 'Software Engineer 2', status: 'Aktif' },
  { id: 3, title: 'Software Engineer 3', status: 'Aktif' },
  { id: 4, title: 'Product Designer', status: 'Seleksi CV' },
];

export default function JobContent({ onContactApplicant }: { onContactApplicant: (application: JobApplication) => void }) {
  const [applications, setApplications] = useState<JobApplication[]>([]);
  const [managedJob, setManagedJob] = useState<CompanyJob | null>(null);
  ...
  const managedApplicants = managedJob ? applicationsByJob[managedJob.id] ?? [] : [];
      ...
      {companyJobs.map((job) => (
        <article key={job.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">{applicationsByJob[job.id]?.length ?? 0} pelamar - {job.status}</p>
          </div>
        ...
        <div className="fixed inset-0 z-[60] bg-black/40 px-4 py-6 flex items-end sm:items-center justify-center">
          <section className="bg-white w-full max-w-xl max-h-[82vh] rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col overflow-hidden">
            <div className="p-4 border-b border-gray-100 flex items-start justify-between gap-3">
              <div>
                <h2 className="text-lg font-bold text-blue-900">{managedJob.title}</h2>
